...
- Baseline commit: `125725c10081c4b3511ffc5aaf1b8e0836434bd8`
...
The reproduction controller writes to a separate Drive location and does not overwrite the current `stage_output` directory:
...
Each stage can also be run separately by setting `MFAR_CODE_ROOT` to the cloned repository and `MFAR_GDRIVE_ROOT` to the selected run directory before opening the corresponding notebook.
...
python tools/create_baseline_manifest.py snapshot \
  --repo-root . \
  --drive-root /path/to/isolated/run_a \
  --output-dir /path/to/isolated/run_a/baseline_evidence \
...
python tools/create_baseline_manifest.py compare \
  --first /path/to/run_a/baseline_evidence/baseline_manifest.json \
  --second /path/to/run_b/baseline_evidence/baseline_manifest.json \
